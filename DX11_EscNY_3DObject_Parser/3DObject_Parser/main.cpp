...
// Everything was moved to Includes.h
...
{
	// Initialize common controls, used for extended window styles and
	// other extended window types like the progress bar
	INITCOMMONCONTROLSEX ctrls{};
	
	// Flag stating I want to initialize a progress bar
	ctrls.dwICC = ICC_PROGRESS_CLASS;
	
	// Size of the current version of this init struct, Windows does this a lot
	// It uses the size of the struct you are passing to determine which 
	// version is being uses.
	ctrls.dwSize = sizeof( INITCOMMONCONTROLSEX );

	// Initialize the common controls using the filled in init struct
	InitCommonControlsEx( &ctrls );

	// Window is created in the constructor of Application
	Application app;

	// Application::Main is where the message loop runs
	app.Main();
...
LRESULT CALLBACK WndProc( HWND WinHandle, UINT Message, WPARAM wParam, LPARAM lParam )
{
	...
	{
		pApp = reinterpret_cast<Application *>(
			reinterpret_cast<LPCREATESTRUCT>( lParam )->lpCreateParams );
		pApp->OnNonClientCreate( WinHandle );
	}
	...
	if( pApp )
	{
		result = pApp->MessageHandler( Message, wParam, lParam );
		return result;
	}
